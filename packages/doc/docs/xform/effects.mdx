...
在 mobx 的基础上，xform 额外提供了 Form.Effect 与 AsyncValue，前者用于简化 reaction 的书写，后者用于处理异步数据源。
...
我们可以通过 [`AsyncValue`](api#asyncvalue) 来创建异步数据源：
...
`new AsyncValue` 会根据参数 fetcher 创建出一个 [`AsyncValue`](api#asyncvalue) 对象，当 `asyncValue.current` 被订阅时，fetcher 将被调用，其返回结果将作为 `asyncValue.current` 的值，而 `asyncValue.status` 会记录目前异步数据的加载状态（loading / ready / error)。
...
:::info
xform AsyncValue 参考了 [recoil async selector](https://recoiljs.org/docs/api-reference/core/selector#asynchronous-selectors)。 允许开发者使用同步的代码来描述一个异步函数的返回结果，非常适合抽象表单中的异步下拉数据源。
:::
