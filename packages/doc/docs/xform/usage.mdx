...
当需要创建与组件生命周期一致的 FormModel 时，我们推荐以下的方式：为组件创建一个单独的 Store，然后在组件中通过 useState 或 [useLocalObservable](https://mobx.js.org/api.html#uselocalobservable) 创建一个 Store 实例。
