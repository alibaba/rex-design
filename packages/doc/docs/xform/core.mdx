...
针对异步数据源的场景，xform 参考了 [recoil async selector](https://recoiljs.org/docs/api-reference/core/selector#asynchronous-selectors) 设计，提供了 AsyncValue，允许开发者使用同步的代码来描述一个异步函数的返回结果，非常适合抽象表单中的异步下拉数据源，具体详见 [xform 联动](effects#异步数据源).
