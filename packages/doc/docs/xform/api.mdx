...
- `Feild` 监听对应 Field 的 value 变化
- `Array<string | Field>` 监听多个「数据索引」或「字段」的 value 变化；effect 被调用时，将用一个数组包裹这些数据索引或字段的值
...
<PropsTable
  props={[
    ...
    { name: 'required', type: 'boolean', description: '是否展示「*」' },
...
- `field.getFork(forkName)` 获取 field 的 fork，详见 [](advanced#) 用于在视图中多次渲染「同一个数据索引」
...
`makeAsyncValue<T>(fetcher: () => Promise<T>, initValue?: T): AsyncValue<T>`

基于 `fetcher` 创建定义一个 AsyncValue，用于声明式地定义异步数据源。

当 `asyncValue.current` 或 `asyncValue.status` 或 `asyncValue.error` 被订阅时， fetcher 将被首次调用， 其返回结果将作为 `asyncValue.current` 的值。
