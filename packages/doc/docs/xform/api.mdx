...
<PropsTable
  props={[
    ...
    { name: 'label', required: true, description: '字段的显示名称' },
    { name: 'defaultValue', description: '默认值' },
    { name: 'componentProps', description: '组件的 props' },
    { category: '校验', name: 'validate', type: '(value: any) => any', description: '校验方法' },
    { category: '校验', name: 'required', description: '字段是否必填' },
    ...
    { category: '{🚀}', name: 'items', description: '测试用，即将被移除，勿使用' },
  ]}
/>
...
创建表单联动，每当 watch 对应的值发生变化时，effect 都会被调用。
...
<PropsTable
  props={[
    { name: 'defaultColumnWidth', type: 'number' },
    { name: 'operationColumn', type: 'Partial<Column>' },
    { name: 'orderColumn', type: 'Partial<Column>' },
...
Model 表示「一个对象或数组对应的表单模型部分」。

- `model.getSubModel(name): Model` 获取一个子模型
- `model.getSubArray(name): Model` 获取一个子模型，但获取的子模型的数据格式为数组
...
- `model.id: string` 模型在所属 RootModel 内的唯一 id
...
- `field.state` 字段状态，这是一个 mobx observable 对象，默认为 `{}`。
  - 目前，我们约定了 `field.state.error` 作为该字段上的错误状态，当 state.error 有值时，页面中就会展示对应的出错提示。
...
mobx action. 执行 model 上所有的校验，并返回所有的报错信息

### `fieldUtils.validate`

`fieldUtils.validate(field: Field): any`

mobx action. 执行指定字段上的校验，返回报错信息。
