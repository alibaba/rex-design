...
- `model.getTupleField(...names): Field` 获取[多个 name 组合而成的元祖字段](advanced#数组解构)
  - 注意该方法暂不支持使用 `.` 来表示嵌套数据索引
...
`new AsyncValue<T>(fetcher: () => Promise<T>, initValue?: T)`

基于 `fetcher` 创建定义一个 AsyncValue，用于声明式地定义异步数据源。

当 asyncValue 的 value/status/error 被订阅时，fetcher 将被首次调用，其返回结果将作为 `asyncValue.current` 的值。

调用 fetcher 的过程中会进行依赖收集，每当这些依赖的值发生变化时，fetcher 都会被重新触发。 具体规则：
...
4. 当 fetcher 返回的 Promise 仍处于 pending 状态时，若 fetcher 再次被调用，已有的 promise 将被忽略，并回到 1.

<!-- 此外，AsyncValue 的 loading 状态有传染性：当 fetcher 遇到 loadingAsyncValue.current 时，会自动将 -->
